...
Each node has a really simple workflow. It starts running and waits for a request, which could be GET or PUT from the \textit{Frontend} node.
When the node gets a request it checks if is supposed to be for him, if yes then the request will be executed and the response is sent to the Frontend, otherwise the Node forward the request to the next node.
...
To be sure that the node is working on the key that is supposed to be for him, this formula was implemented:
\begin{lstlisting}
...
\begin{figure}[h!]
  \centering
    \includegraphics[width=0.5\textwidth]{get}
    \caption{How the GET works on the nodes}
    \label{fig:get}
\end{figure}

\begin{figure}[h!]
  \centering
    \includegraphics[width=0.5\textwidth]{put}
    \caption{How the PUT works on the nodes}
    \label{fig:put}
\end{figure}
